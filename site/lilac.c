...
const char * const user_definestr[] = {
    ...
    "显示星座及性别",             /* DEF_SHOWDETAILUSERDATA 2003.7.31 */
...
*/
     ...
     */
	if (lvl & PERM_ADMIN)
		strcpy(buf, "本站站长");  /* lanslot :增加站长 2007.07.20 */
...
#define MAX_GUEST_PER_IP 20
    ...
    if ((!HAS_PERM(user, PERM_LOGINOK)) && logincount > 0)
        return 1;
    if(arg.www_count>=1 && mode ==1)
		return 1;
	if((arg.telnet_count >= 2) && (mode == 0))
		return 1;
   // if ( (logincount >= 1)
   ...
   //   return 1;
//	if ( (logincount >= 2)
//			&&!(((arg.telnet_count == 1) && (mode == 0)) /*telnet 个数>=1可以再登一个telnet*/
//				||(((arg.www_count == 0) && (mode == 1)))))  /*user login limit*/
//		return 1;
    return 0;
}
...
const struct userec *urec;
{
	...
	// 2008-02-24
	struct tm *time_str;
	time_t t = urec->lastlogin;
	time_str = localtime(&t);
	if(time_str->tm_year+1900 <= 2007 && time_str->tm_mon+1 <= 6)
		return 1;
