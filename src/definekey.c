...
int load_key(char * fn)
{
    ...
    if(fn==NULL){
		...
		if(!dash(fname))
#endif
		snprintf(fname, STRLEN, "%s/service/definekey.lilac", BBSHOME);
		}
#else
 if (fn == NULL)
...
//By Marvel
#ifdef LILAC
    char sname[5][14]={"水木清华", "一塌糊涂", "北大未名", "水木清华","紫丁香默认"};
    char fname[5][30]={"service/definekey.smth", "service/definekey.ytht", "service/definekey.pku", "service/definekey.free","service/definekey.lilac" };
#else
...
#else
#ifdef LILAC
    char sname[5][14]={"一塌糊涂", "北大未名", "南大小百合", "水木清华", "紫丁香默认"};
    char fname[5][30]={"service/definekey.ytht", "service/definekey.pku", "service/definekey.nju", "service/definekey.smth", "service/definekey.lilac"};
#else
