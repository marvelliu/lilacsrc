...
int gen_x_deny_notify(char *level, char *uid, int deny)
{
	...
	if (deny)
	{
		sprintf(buffer, "%s被封禁 %s ", uid, level);
		rbuff[0] = 0;
		while (!rbuff[0])
		{
			move(0, 0);
			char genbuf[4];
			clear();
			prints("请输入封禁 %s 的 %s 的理由，CTRL + Q 换行", uid, level);
			i = multi_getdata(1, 0, 79, NULL, rbuff, 512, 11, false, 0);
			if ( rbuff[0] ==0 )
			{
				getdata(i + 2, 0, "你没有输入封禁理由,你想重新输入封禁理由吗? (Y)是的, 重新输入 (N)不，不想输入了 [Y]: ", genbuf, 2, DOECHO, NULL, 1);
		...
		sprintf(buffer, "%s被解禁 %s ", uid, level);
	...
	{
		fprintf(fn, "\n封禁说明:\n\n");
		fprintf(fn, "\x1b[4m%s\x1b[m \n\n我很遗憾地通知您，你的 \x1b[4m%s\x1b[m 被取消了!\n", rbuff, level);
		fprintf(fn, "如果您对封禁有任何异议，请到 Complain 版申诉。 \n");
		fprintf(fn, "请您仔细阅读本站rules版相关站规，并在到期后在complain版申请解封。 \n ");
	}
	else
	{
		fprintf(fn, "非常高兴的通知您，你的  \x1b[4m%s\x1b[m 已被恢复！\n", level);
		fprintf(fn, "希望您仔细阅读本站rules版相关规定，以免再次受到类似的处罚。\n");
	}
	fprintf(fn, "\n");
	
	fprintf(fn, 
		"                            " NAME_BBS_CHINESE NAME_SYSOP_GROUP "值班站务：\x1b[4m%s\x1b[m\n", 
	...
	fn = fopen(filename, "w+");
	if (deny)
		fprintf(fn, "\n封禁说明:\n\n\x1b[4m%s\x1b[m\n\n根据以上原因, \x1b[4m%s\x1b[4m 的 \x1b[4m%s\x1b[m 被取消，请到complain版申诉或申请解除封禁。\n\n", rbuff, uid, level);
	else
		fprintf(fn, "\n\x1b[4m%s\x1b[m 的 \x1b[4m%s\x1b[m 被恢复! \n\n", uid, level);

	fprintf(fn, "                            " NAME_BBS_CHINESE NAME_SYSOP_GROUP "值班站务：\x1b[4m%s\x1b[m\n", saveptr->userid);
	fprintf(fn, "                              %s\n", ctime(&now));
    fclose(fn);
	sprintf(rbuff, "【公告】%s", buffer);
