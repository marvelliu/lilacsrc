...
int uinfo_query(struct userec *u, int real, int unum) 
{
    ...
    getdata(t_lines - 1, 0, real ? "请选择 (0)结束 (1)修改资料 (2)设定密码 (3) 改 ID (4) 改积分==> [0]" : "请选择 (0)结束 (1)修改资料 (2)设定密码 ==> [0]", ans, 2, DOECHO, NULL, true);
    clear();
    i = 3;
    move(i++, 0);
    if (ans[0] != '3' || real)
    ...
    case '3':
        if (!real) {
            ...
            getdata(i++, 0, "新的使用者代号: ", genbuf, 16, DOECHO, NULL, true);
	...
	case '4':
		...
		if ( sc >= 0 )
		{
			sprintf( genbuf, "积分增加 %d \n情输入此次积分操作的理由: ", new_score - u->score );
		}
		else
		{
			sprintf( genbuf, "积分减少 %d \n情输入此次积分操作的理由: ", u->score - new_score );
		}
		getdata(i++, 0, genbuf, buf, STRLEN, DOECHO, NULL, true);
    ...
    for (;;)
	{
        ...
        if (*ans == 'y' || *ans == 'Y') {
	     ...
	     if (strcmp(u->userid, newinfo.userid)) {
			...
			{
				sflag = true;
				sprintf( fname, "[公告] 增加 %s 积分 %d 分", u->userid, newinfo.score - u->score );
				post_file(NULL, "", "etc/posts/update_score", "ScoreRecords", fname, 0, 1, getSession());
			}
			else if( u->score > newinfo.score )
			{
				sflag = true;
				sprintf( fname, "[公告] 减少 %s 积分 %d 分", u->userid, u->score - newinfo.score );
				post_file(NULL, "", "etc/posts/update_score", "ScoreRecords", fname, 0, 1, getSession());
			}
            ...
            if (real)
			{
                ...
                if (strcmp(u->userid, newinfo.userid))
					...
					sprintf( secu, "修改 %s 的积分", u->userid );
				else
...
int x_fillform(void){
	...
	if(!lilac_validmail(getSession()->currentmemo->ud.reg_email)){
        prints("Email没有被验证，所以您还无法填写注册信息\n");
		getdata(t_lines - 1, 0, "重新发送验证信件 (Y/N)? [N]: ", ans, 3, DOECHO, NULL, true);
		if(ans[0]!='y' && ans[0]!='Y')
        	return -1;
        genbuf[0] = '\0';
        while (!lilac_validmail(genbuf) ) {
            getfield(10, "用于激活你的ID", "Email", getSession()->currentmemo->ud.email, STRLEN);
        }
        verify_email(getSession());
		prints("一封激活信已经发送到您的信箱(%s)，注意查收. ", genbuf);
        pressreturn();
        return 0;
