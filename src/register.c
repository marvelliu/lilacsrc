    ...
    int msize;
{
	...
	if (dashf(genbuf))
	{
       /*if (lilac_auto_register(userid, email, msize, getSession()) < 0)    // 完成自动注册
	   {
           move(t_lines - 2, 0);
		   prints("恭喜你，你的帐号已经激活!");
		   pressanykey();
...
//		write_userdata(getCurrentUser()->userid,&curruserdata);
	...
	if(!lilac_validid(getSession()->currentmemo->ud.ICQ)
		...
		do{ 
			getdata(8, 0, "您的身份证号: << ", buf, STRLEN, DOECHO, NULL, true);
	...
	*/
	...
	if(!lilac_validmail(getSession()->currentmemo->ud.reg_email) 
		...
		if(!verify_email_file_exist(getSession()) )
			prints("尊敬的用户：由于社区要提供密码找回功能，\n所以您需要提供一个真实的邮箱，\n请您在登录之后，选择\n［个人工具栏］->［激活信箱］\n");
		else
			prints("尊敬的用户：您尚未激活您的信箱，请在请您在登录之后，\n重新选择［个人工具栏］->激活信箱\n");
		pressanykey();
	}
...
//					(curruserdata.gender == 'M') ? "小弟" : "小女子");
    ...
    if (!(getCurrentUser()->userlevel & PERM_LOGINOK)) {
		...
		//By Marvel 用PERM_LOGINOK权限判断
		// 这部分是什么意思？暂时去掉吧？？ MARVEL
		//struct userec *lookupuser;
		//getuser(getCurrentUser()->userid, &lookupuser);
		//getCurrentUser()->userlevel |= (PERM_DEFAULT | PERM_NOIPCTRL );
	}
