         ...
         */
{
	...
	if(strcmp(currboard->filename,"Complain")!=0 || !chk_currBM(currboard -> BM, getCurrentUser()))
	{
	    ...
	    prints("\t\t同志们，朋友们！快捷键回收啦！\n\n\t\t您刚才按的键已经被Complain的ZT私吞。\n\n");
    	prints("\t\t\033[1;32m请使用Ctrl+G 2进入同主题阅读\033[0m\n");
	...
	if(strstr(tmpbuf,"已处理]")!=NULL && strstr(tmpbuf,"[")!=NULL){
		return PARTUPDATE; 
	}

	sprintf(buf,"[%s已处理]:%s",getCurrentUser()->userid,tmpbuf);
