...
void g_edits()
{
     ...
     * 下面的21是行号，以后添加，可以相应调整
     */
    getdata(21, 0, "你要编修哪一项系统画面档案", ans, 3, DOECHO, NULL, true);
    ch = atoi(ans);
    if (!isdigit(ans[0]) || ch <= 0 || ch > num || ans[0] == '\n' || ans[0] == '\0')
        return;
    ch -= 1;
...
int merge_favboard(void){
	//see src/register.c:113, passbuf has STRLEN chars
	char userid[IDLEN+2],passbuf[STRLEN], typebuf[10];
	struct userec *user;
    int oldmode,ret;
	int tuid=0;
	int brdlist_t1=0, brdlist_t2=0;
	int type;
	struct favbrd_struct brdlist1[FAVBOARDNUM], brdlist2[FAVBOARDNUM];
	oldmode = uinfo.mode;
    modify_user_mode(MERGEFAVBRD);

	clear();
    move(0,0);
    prints("\033[1;32m%s\033[m","[合并收藏夹]");


	do{
        move(1,0);
        clrtobot();
        getdata(2,0,"请输入需要合并的用户名: ",userid,16,DOECHO,NULL,true);
        if(!userid[0])
            break;
        move(3,0);
		if (!(tuid = getuser(userid, &user))) {
			prints("没有该用户\n");
	        break;
		}
        getdata(3,0,"请输入需要合并的用户密码: ",passbuf,39,NOECHO,NULL,true);
        if(!passbuf[0])
            break;
		ret = checkpasswd2(passbuf, user);
        move(4,0);
		if(!ret){
            prints("\033[1;31m%s\033[0;33m<Enter>\033[m","用户名密码错误...");
	        break;
		}
        getdata(4,0,"导入时是否覆盖原有收藏夹？(Y/N): ",typebuf,1,DOECHO,NULL,true);
        if(!typebuf[0])
            break;
        move(5,0);
		type = (typebuf[0]=='Y'||typebuf[0]=='y')?1:0;
		prints("您的选择为导入%s的收藏夹, 导入时%s覆盖%s的收藏夹，",
				userid, (typebuf[0]=='Y'||typebuf[0]=='y')?"":"不",getCurrentUser()->userid);
		getdata(6,0,"确定吗(Y/N)",typebuf,1,DOECHO,NULL,true);
		if(!(typebuf[0]=='Y'||typebuf[0]=='y'))
			continue;

 		load_myboard1(user, brdlist2, &brdlist_t2, 1);

		if(type != 1){
			load_myboard1(getCurrentUser(), brdlist1, &brdlist_t1, 1);
			if(brdlist_t1+brdlist_t2>FAVBOARDNUM){
				prints("对不起，您和您要合并的收藏夹目录总和过多，请删减后小于%d后重新合并", FAVBOARDNUM);
				break;
			}
		}
		merge(brdlist1, brdlist_t1, brdlist2, brdlist_t2, getSession()->mybrd_list, &getSession()->mybrd_list_t, user->userid, type);
		if(getSession()->mybrd_list_t<=0){
			prints("合并失败\n");
			break;
		}
    	char fname[MAXPATH];
		sethomefile(fname, getCurrentUser()->userid, "favboard");
		save_favboard1(fname, getSession()->mybrd_list, &getSession()->mybrd_list_t);
		prints("合并结束\n");
		break;
    }
    while(1);
...
int active_mail()
{
	...
	if(!lilac_validmail(getSession()->currentmemo->ud.email)){
		if(lilac_validmail(getSession()->currentmemo->ud.realemail))
			strncpy(getSession()->currentmemo->ud.email, getSession()->currentmemo->ud.realemail, STRLEN);
		
		if(lilac_validmail(getSession()->currentmemo->ud.reg_email))
			strncpy(getSession()->currentmemo->ud.email, getSession()->currentmemo->ud.realemail, STRLEN);
	}

	prints("您的邮箱为%s, ", getSession()->currentmemo->ud.email);
	getdata(7, 0, "向上面的邮箱发送激活信件吗？(Y/N) ", ans, 2, DOECHO, NULL, true);
	if(!(*ans == 'Y' || *ans == 'y')){
		do{
			getdata(8, 0, "您的真实mail[用于激活id]: << ", buf, STRLEN, DOECHO, NULL, true);
			strncpy(getSession()->currentmemo->ud.email, buf, STRLEN);
		}while (!lilac_validmail(getSession()->currentmemo->ud.email) ); 
	}
	verify_email(getSession());
	prints("一封激活信已经发送到您的信箱(%s)，注意查收. ", getSession()->currentmemo->ud.email);
	pressanykey();
	return 0;
}
#endif
