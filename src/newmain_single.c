...
/*Haohmaru.98.11.3*/
		...
		/*
        // 2007年由Marvel添加，禁止guest进入
		//2007年12月5号去掉
        if (!strcmp(uid,"guest")) {
	        prints("现在系统禁止guest进入，请注册或者登陆");
	        oflush();
	        sleep(1);
	        exit(1);
		}
		*/
        if (strcasecmp(uid, "new") == 0) {
        	clear();
			move(1,0);
			prints("您好，请点击http://www.lilacbbs.com/bbs/member.php?mod=register.php进行注册");
        	pressanykey();
            exit(1);
#ifdef LOGINASNEW
...
//    read_userdata(getCurrentUser()->userid, &curruserdata);
    ...
    //2007-06-06 by Marvel ，凡是没有“IP无限制”权限的老用户都禁止入内
    if (!HAS_PERM(getCurrentUser(), PERM_NOIPCTRL) && HAS_PERM(getCurrentUser(), PERM_LOGINOK)) {        
        //prints("\033[32m本帐号的IP受到限制，请激活。请向 \033[36mSYSOP\033[32m 查询原因\033[m\n");
...
void update_endline()
{
	...
	if(recmd==0){
		...
		if(get_recommend_info(tmp, info)) /* tmp is 0 ~ (RECMD_INFO_NUM-1) */
			sprintf(stitle, "\033[1;44;33m温馨提示：[\033[36m%.60s\033[33m]\t[\033[36m%4d\033[33m人/\033[1;36m%3d\033[33m友][\033[36m%.12s\033[33m]", 
