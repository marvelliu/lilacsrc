...
/*    search_ulist( &uin, t_cmpuids, tuid );*/
...
#else
    uleveltochar(permstr, lookupuser);
    prints("信箱：[\033[5m%2s\033[m] 生命力：[%d] 身份: [\033[1;32m%s\033[m]%s\n",
	...
	if( ! DEFINE(lookupuser, DEF_SHOWDETAILUSERDATA) || read_userdata(uident,&udata) ){
		...
		clr = (udata.gender == 'F') ? 5 : 6 ;
		sprintf(horobuf, "星座：[\033[1;3%dm%s座\033[m]", clr, horoscope(udata.birthmonth, udata.birthday) );
		//sprintf(horobuf, "性别：[\033[1;3%dm%s\033[m]", clr, (udata.gender == 'F')?:"女":"男" );
		prints("%s\t",horobuf);
	}
#endif

#ifdef LILAC
	prints("经验值: [%d](%s)\t",exp,c_exp(exp));
