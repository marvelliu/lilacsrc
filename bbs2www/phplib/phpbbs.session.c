...
/**
 * 特殊用途，用来充当其它WEB程序向term转移论坛数据时使用
 * by: kaede
 */
PHP_FUNCTION(bbs_setuser_poster_nologin) {
    struct userec *user;
    char *username;
    int len;
    int num;
        if (zend_parse_parameters(1 TSRMLS_CC, "s", &username,&len) != SUCCESS) {
            WRONG_PARAM_COUNT;
        }
    num = getuser(username,&user);
    setcurrentuser(user,num);
}
