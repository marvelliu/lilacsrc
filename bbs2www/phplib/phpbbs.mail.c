...
PHP_FUNCTION(bbs_postmail){
	...
	{
		if(zend_parse_parameters(6 TSRMLS_CC, "ss/s/lll", &recvID, &idLen,&title,&tLen,&content,&cLen,&sig,&backup,&forceuncheck) == FAILURE)
		{
			WRONG_PARAM_COUNT;
		}
	}
...
#ifdef LILAC
    if (check_last_post_time(getSession()->currentuinfo) && forceuncheck > 0) {
#else
