...
* 3 用户名已经存在
* -1 用户头像太宽
* -2 用户头像太高 这个返回值还代表另一个信息，因为没使用过所以不知道，有谁发现了请填上。
* -5 身份证长度不是 15 或 18
* 0 成功
*
*
...
*/
	...
	if (userface_url_len!=0) {
		userface_img=-1;
		if ( (userface_width<0) || (userface_width>150) ){
			RETURN_LONG(-1);
		}
		if ( (userface_height<0) || (userface_height>255) ){
			RETURN_LONG(-2);
		}
	} else {
		userface_width=0;
...
#ifdef LILAC
	if(ud.reg_email[0] == '\0'){
		verify_email_by_info(realname, userid, email);
        RETURN_LONG(20);
	}
	else{
        RETURN_LONG(0);
...
PHP_FUNCTION(bbs_modify_info)
{
	...
	long	gender,
	        ...
	        month,
			day;
    if (ac != 8 || zend_parse_parameters(8 TSRMLS_CC, "ssssllll", &username, &username_len,&realname,&realname_len,&address,&address_len,&email,&email_len, &gender, &year, &month, &day) == FAILURE)
#else
...
#ifdef LILAC
	newuser.userlevel |= PERM_NOIPCTRL;
#endif
...
PHP_FUNCTION(bbs_verified_email_file_exist)
{
	...
	if(getuser(userid,&uc)==0)
		RETURN_LONG(-1);
	int ret = verified_email_file_exist_by_userid(userid);
	RETURN_LONG(ret);
}
...
PHP_FUNCTION(bbs_verify_email_file_exist)
{
	...
	if(getuser(userid,&uc)==0)
		RETURN_LONG(-1);
	int ret = verify_email_file_exist_by_userid(userid);
	RETURN_LONG(ret);
}
...
PHP_FUNCTION(bbs_send_verify_email)
{
	...
	if (ac != 2 || zend_parse_parameters(ZEND_NUM_ARGS()TSRMLS_CC, "ss" , &userid , &userid_len, &email, &email_len) == FAILURE)
	{
		WRONG_PARAM_COUNT;
	}
	if(getuser(userid,&uc)==0)
		RETURN_LONG(-1);
	read_userdata(userid, &ud);
	strncpy(ud.email, email, sizeof(ud.email));
	write_userdata(userid, &ud);
	int ret = verify_email_by_info(ud.realname, userid, email);
	RETURN_LONG(ret);
}
